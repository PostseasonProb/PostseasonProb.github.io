...
for i in range (0,noteams):
    fig.add_trace(go.Scatter(x=date[i],y=PS_odd[i],mode='lines',name=team[i], line=dict(width=4,color=tcolors[i])))
...
#py.plot(fig,filename='PS_2020.html',auto_open=True,config=dict({'displayModeBar' : False}))
pio.write_html(fig, file=str(pathlib.Path().resolve())+'/PS_2020.html',auto_open=False,config=config)
