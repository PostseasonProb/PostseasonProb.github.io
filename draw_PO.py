...
PO_odd = np.zeros((10,length),dtype=float)
...
for x in range (0,noteams):
    ...
    for y in range (0,length):
        ...
        PO_odd[x][y] = percentage[x][1][y]/100000.
...
for i in range (0,noteams):
    fig.add_trace(go.Scatter(x=date[i],y=PO_odd[i],mode='lines',name=team[i], line=dict(width=4,color=tcolors[i])))
...
py.plot(fig,filename='PO_2020.html',auto_open=False,config=config)
#pio.write_html(fig, file=str(pathlib.Path().resolve())+'/include/PO_2021.html',auto_open=False,config=config)
